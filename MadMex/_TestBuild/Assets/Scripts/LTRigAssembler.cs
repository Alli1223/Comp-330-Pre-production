...
public class LTRigAssembler : ScriptableObject 
{
	...
	{
        GameObject go = TorsoAssembler(LTAssetNameToID(torsoAssetName), LGAssetNameToID(legAssetName), atPosition, atRotation);
	...
	public int LTAssetNameToID(string name)
	{
		for (int i = 0; i < LTData.Length; i++)
		{
			if (name == LTData[i].asset.name)
			{
				return i;
			}
		}
		Debug.LogError("Asset named: " + name + " | Has not been found");
		return 0;
	}

	public int LGAssetNameToID(string name)
	{
		for (int i = 0; i < LGData.Length; i++)
		{
			if (name == LGData[i].asset.name)
			{
				return i;
			}
		}
    ...
    void OverrideAnimationClips(int torsoID, int legID, LegType legType)
    {
        ...
        {
            Debug.LogWarning("Animation override is missing its animations for 4 joint legs");
            animOverride[oc[1].name] = ThreeJointLeg_IDLE;
            animOverride[oc[2].name] = ThreeJointLeg_walk_light;
            animOverride[oc[3].name] = ThreeJointLeg_run_light;
        }
